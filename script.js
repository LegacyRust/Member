...
async function main() {
  try {
    const text = await fetch(CSV_URL + "&_=" + Date.now(), { cache: "no-store" }).then((res) => {
      if (!res.ok) throw new Error("CSVを読み込めませんでした");
      return res.text();
    });

    const rows = parseCSV(text);
    const people = rows.map(normalizeRow).filter((row) => row.name);
...
function normalizeRow(row) {
  return {
    type: getCell(row, "type").toLowerCase(),
    name: getCell(row, "name"),
    icon: getCell(row, "icon"),
    x: getCell(row, "x"),
    twitch: getCell(row, "twitch"),
...
function getCell(row, key) {
  ...
  for (const actualKey of Object.keys(row)) {
    if (String(actualKey).trim().toLowerCase() === wanted) {
